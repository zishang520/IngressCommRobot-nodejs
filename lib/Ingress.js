...
(function() {
    ...
    const request = require('request'),
        Conffs = require(__dirname + '/Config.js'),
    ...
    Ingress.prototype.curl = function(url, body, headers, call) {
        ...
        // 设置cookie
        option.jar = request.jar(new FileCookieStore(COOKIE_FILE, {
            sync: true
        }));
    ...
    Ingress.prototype.getToken = function() {
        ...
        let matchs = cookie.getCookieString('https://intel.ingress.com/intel').match(/csrftoken=(\w+)(?=;)?/im);
    ...
    Ingress.prototype.getV_token = function(call) {
        ...
        let auto_login = (login_url) => {
            ...
            let jump_login_page = (data) => {
                let $ = cheerio.load(data);
                let $url = 'https://accounts.google.com/ServiceLogin?continue=https%3A%2F%2Fappengine.google.com%2F_ah%2Fconflogin%3Fcontinue%3Dhttps%3A%2F%intel.ingress.com%2Fintel&rip=1&nojavascript=1&service=ah&ltmpl=gm';
        ...
        this.curl('https://intel.ingress.com/intel', undefined, undefined, (err, res, data) => {
